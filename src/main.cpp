...
// consensus bug incident
int NUKEHEIGHT = 242467;
static bool DoNuke(const CBlockIndex* pindexPrev) {
    if (fRequireStandard)
        return pindexPrev->nHeight > NUKEHEIGHT;
    return false;
}
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, std::set<valtype>& expiredNames, bool fJustCheck)
{
    ...
    if (block.nVersion.GetBaseVersion() >= 3 && (DoNuke(pindex->pprev) || IsSuperMajority(3, pindex->pprev, chainparams.GetConsensus().nMajorityEnforceBlockUpgrade, chainparams.GetConsensus()))) {
...
bool ContextualCheckBlockHeader(const CBlockHeader& block, CValidationState& state, CBlockIndex * const pindexPrev)
{
    ...
    if (block.nVersion.GetBaseVersion() < 2 && (DoNuke(pindexPrev) || IsSuperMajority(2, pindexPrev, consensusParams.nMajorityRejectBlockOutdated, consensusParams)))
        return state.Invalid(error("%s: rejected nVersion=1 block", __func__),
                             REJECT_OBSOLETE, "bad-version");

    // Reject block.nVersion=2 blocks when 95% (75% on testnet) of the network has upgraded:
    if (block.nVersion.GetBaseVersion() < 3 && (DoNuke(pindexPrev) || IsSuperMajority(3, pindexPrev, consensusParams.nMajorityRejectBlockOutdated, consensusParams)))
...
bool ContextualCheckBlock(const CBlock& block, CValidationState& state, CBlockIndex * const pindexPrev)
{
    ...
    if (block.nVersion.GetBaseVersion() >= 2 && (DoNuke(pindexPrev) || IsSuperMajority(2, pindexPrev, consensusParams.nMajorityEnforceBlockUpgrade, consensusParams)))
    {
